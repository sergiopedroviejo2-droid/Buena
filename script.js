document.addEventListener('DOMContentLoaded', () => {
    ...
    function populateSelect(select, items, placeholderText) {
        const label = placeholderText || select.querySelector('option[value=""]')?.textContent || 'Seleccionar...';
    ...
    async function loadDropdowns() {
        const [treatments, professionals, rooms] = await Promise.all([
            fetchApi('/treatments'),
            fetchApi('/professionals'),
            fetchApi('/rooms')
        ]);

        document.querySelectorAll('select[name="id_tratamiento"]').forEach((select) => {
            const inAppointment = !!select.closest('#appointmentModal');
            populateSelect(
                select,
                treatments,
                inAppointment ? 'Seleccionar tratamiento' : 'Todos los tratamientos'
            );
        });

        document.querySelectorAll('select[name="id_profesional"]').forEach((select) => {
            const inAppointment = !!select.closest('#appointmentModal');
            populateSelect(
                select,
                professionals,
                inAppointment ? 'Seleccionar profesional' : 'Todos los profesionales'
            );
        });

        document.querySelectorAll('select[name="id_sala"]').forEach((select) => {
            const inAppointment = !!select.closest('#appointmentModal');
            populateSelect(
                select,
                rooms,
                inAppointment ? 'Seleccionar sala' : 'Todas las salas'
            );
        });
    }
