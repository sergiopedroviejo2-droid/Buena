document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderAppointments() {
        ...
        if (currentCalendarView === 'agenda') {
            html += `<div class="inventory-container glass" style="overflow: hidden;">
            ...
            weekAppts.forEach(a => {
                ...
                html += `
                    <tr>
                        <td>
                            <div style="font-weight: 600; color: var(--primary-color);">${formattedDate}</div>
                            <div style="font-size: 0.75rem; color: var(--secondary-color);"><i class="fa-regular fa-clock" style="margin-right: 0.2rem;"></i>${a.hora.substring(0, 5)} (${a.duracion}m)</div>
                        </td>
                        <td style="font-weight: 500;">${a.paciente_nombre} ${a.paciente_apellidos}</td>
                        <td>${a.tratamiento_nombre}</td>
                        <td>${a.profesional_nombre}</td>
                        <td><span style="color: ${roomColor}; font-weight: 600;"><i class="fa-solid fa-door-open" style="margin-right:0.25rem;"></i>${a.sala_nombre}</span></td>
                        <td><span class="status-badge status-${a.id_estado || a.estado} ${a.estado === 'en_espera' ? 'status-en-espera' : ''}">${a.estado.replace('_', ' ')}</span></td>
                        <td>
                            <div style="display: flex; gap: 0.4rem;">
    ...
    const revenueData = { 'diaria': '$0.00', 'semanal': '$0.00', 'mensual': '$0.00', 'anual': '$0.00' };
    ...
    revenueTabs.forEach(tab => {
        tab.addEventListener('click', () => {
            ...
            revenueAmount.innerText = revenueData[period];
    ...
    if (toggleCalendarViewBtn) {
        toggleCalendarViewBtn.addEventListener('click', () => {
            ...
            toggleCalendarViewBtn.innerHTML = currentCalendarView === 'cards' ? '<i class="fa-solid fa-list"></i> Vista Agenda' : '<i class="fa-solid fa-grip"></i> Vista Tarjetas';
            renderAppointments();
        });
    }
