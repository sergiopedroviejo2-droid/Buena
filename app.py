...
_facturas_has_notas_column = None

def facturas_supports_notas():
    """Detect whether facturas.notas exists in Supabase (schema cache)."""
    global _facturas_has_notas_column
    if _facturas_has_notas_column is None:
        try:
            supabase.table("facturas").select("notas").limit(1).execute()
            _facturas_has_notas_column = True
        except Exception:
            _facturas_has_notas_column = False
    return _facturas_has_notas_column
...
def create_invoice():
    ...
    try:
        ...
        if notas and facturas_supports_notas():
            insert_data['notas'] = notas
